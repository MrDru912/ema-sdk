...
exports.EMAMedicineMapper = void 0;
...
class EMAMedicineMapper {
    constructor(csvPath = 'assets/medicines_output_medicines_en.csv', autoUpdate = true) {
        ...
        this.nameToIdsMap = new Map();
        this.loaded = false;
        this.lastCheckedUpdate = null;
        this.updateCheckIntervalDays = 7;
        // Search indexes
        this.normalizedNameIndex = new Map();
        this.normalizedSubstanceIndex = new Map();
    ...
    async isDataUpToDate() {
        if (!(0, fs_1.existsSync)(this.csvPath)) {
        ...
        try {
            const hasNewData = await (0, ema_data_updater_1.checkForUpdates)(this.csvPath);
            if (hasNewData) {
            ...
            return (0, fs_1.existsSync)(this.csvPath);
        }
    }
    /**
    ...
    async ensureDataLoaded() {
        ...
        if (this.autoUpdate && !(await this.isDataUpToDate())) {
            const updated = await (0, ema_data_updater_1.updateEMAData)(this.csvPath);
            if (!updated) {
    ...
    loadMap() {
        try {
            ...
            const data = (0, fs_1.readFileSync)(this.csvPath, 'utf8');
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                const normalizedName = (0, utils_1.normalizeText)(productName);
                ...
                const id = `${normalizedName}_${(0, utils_1.normalizeText)(country)}_${(0, utils_1.normalizeText)(mah)}`.substring(0, 100);
                // Store medicine
                this.medicineMap.set(id, details);
                // Index by name
                ...
                if (activeSubstance) {
                    const normalizedSubstance = (0, utils_1.normalizeText)(activeSubstance);
                ...
                // Index by country
                if (country) {
                    const normalizedCountry = (0, utils_1.normalizeText)(country);
     ...
     */
    async getPaginatedMedicines(page = 1, pageSize = 20, query, threshold = 70) {
        await this.ensureDataLoaded();
        page = Math.max(1, page);
        pageSize = Math.max(1, Math.min(100, pageSize));
        let filtered = [];
        if (query && query.trim()) {
            const normalizedQuery = (0, utils_1.normalizeText)(query.trim());
            ...
            if (matchedIds.size < pageSize * 2) {
                for (const [id, details] of this.medicineMap.entries()) {
                    ...
                    const fuzzyScore = (0, fuzzball_1.token_sort_ratio)(normalizedQuery, (0, utils_1.normalizeText)(name));
        ...
        const items = pageItems.map(([id, details, score]) => ({
     ...
     */
    async getQuickMedicineMatch(query, threshold = 70) {
        await this.ensureDataLoaded();
        const normalizedQuery = (0, utils_1.normalizeText)(query.trim());
        // 1. Exact name match
        if (this.nameToIdsMap.has(normalizedQuery)) {
    ...
    async getMedicinesByCountry(country, page = 1, pageSize = 20) {
        ...
        const items = pageItems.map(([id, details]) => ({
            name: details.name_of_medicine || details.product_name || details.product_short_name || details.name || '',
            id,
            score: 0,
            data: details
     ...
     */
    async forceUpdate() {
        const success = await (0, ema_data_updater_1.updateEMAData)(this.csvPath);
        if (success) {
            this.loaded = false;
            this.medicineMap.clear();
            this.nameToIdsMap.clear();
...
exports.EMAMedicineMapper = EMAMedicineMapper;
//# sourceMappingURL=ema-medicine-mapper.js.map